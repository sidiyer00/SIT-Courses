...
;; 3 PTS
(define (injective? R)
  (if (= (length (domain (converse R))) (length (make-set (domain (converse R))))) #t #f)
  )

(define (domain R)
  (if (null? R) '()
      (append (list (car (car R))) (domain (cdr R)))
      )
  )
