...
;; 1 PTS
(define (pred p)
  (if (not (pz? p)) (car p) (error-predecessor))
  )
...
;; 2 PTS
(define (p* a b)
  (if (my-or (pz? a) (pz? b)) z
      (p*-helper (pred a) b b)
  )
)

(define (p*-helper counter originalb b)
  (if (pz? counter) b
      (p*-helper (pred counter) originalb (p+ originalb b))
      )
  )
...
(define (p^ a b)
  (cond [(pz? b) (succ z)]
        [(pz? a) z]
        [else (p^-helper a a (pred b))]
        )
  )

(define (p^-helper oribinala a counter)
  (if (pz? counter) a
      (p^-helper oribinala (p* oribinala a) (pred counter))
...
(define (p= a b)
  (cond [(my-and (pz? a) (pz? b)) #t]
        [(my-and (pz? a) (not (pz? b))) #f]
        [(my-and (pz? b) (not (pz? a))) #f]
...
;; 3 PTS
(define (p-prime? p)
  (cond [(my-or (pz? p) (p= p (succ z) ) ) #f]
        [(p= p (succ (succ z))) #t]
        [else (p-prime-helper? (pred p) p)]
  ))

(define (p-prime-helper? num p)
  (cond [(my-or (pz? num) (p= num (succ z) ) ) #t]
        [(p-divides? num p) #f]
        [else (p-prime-helper? (pred num) p)]
  )
  )
