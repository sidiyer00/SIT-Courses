...
(define (compose S R)
  (if (null? S) '()
      (make-set (append (composeHelper (car S) R) (compose (cdr S) R) ))
  ))



(define (composeHelper a b)

  (if (null? b) '()

      (if (equal? (cadar b) (car a))
          (cons (list (caar b) (cadr a)) (composeHelper a (cdr b)))
          (composeHelper a (cdr b))
          )
...
(define (composePower relation power k)
  (if (equal? k 1)
      power
      (composePower relation (compose relation power) (- k 1))))
...
(define (transitive-closure R)
  (if (null? R) '()
      (transitive-helper R (length R))) 
  )

(define (transitive-helper relation len) 
  (if (= len 0) '()
      (union (power len relation) (transitive-helper relation (- len 1))
             )))
...
(define (equivalence? n R)
  (and (transitive? R) (symmetric? R) (reflexive? n R)))
...
(define (surjective? n R)
  (surjective-helper n (map (lambda (x) (car (reverse x))) R))

  )

(define (surjective-helper k S)
  (if (= k 0) #t
      (if (member k S) (surjective-helper (- k 1) S) #f)
      ))
