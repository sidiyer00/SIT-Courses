...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        // pseudocode from Michael Sambol - YouTube
        const K& key = key_value.first;
        Node<K, V> *x, *y;
        
        // either start at root node or some intermediate node
        if (it != end()) {
            x = it.node_ptr;
            y = x->parent;
        } else {
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
        ...
        n->parent = nullptr;    // remove connection to parent
        n->left = nullptr;      // remove connection to left
        delete n->left;         // delete left
        n->right = nullptr;     // remove connection to right
        delete n->right;        // delete right
     ...
     * 
     * ISN'T THIS BASICALLY LEAF COUNT?
     */
